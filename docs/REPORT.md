...
- **验收判定项（项数随平台能力裁剪，实测 B300 为 14 项）**：GPU PCIe / NVLink（v1.48.40 能力感知：消费级无 NVLink 判 N/A）/ DCGM（消费级 N/A）/ VBIOS / 内存速率 / IB 线缆 / 磁盘寿命 / SMART / 电源冗余 / 温度 / ****风扇（v1.48.88：`FAN_DATA_OK` 区分「采集失败」与「平台无风扇」——命令超时致日志为空时显示 `N/A（未取到数据）` 并计入数据不足，不再误报「无风扇」）**；**CPU 功耗（v1.48.89：采集端读 RAPL 两次采样算 ΔE/Δt，报告端单列，可与 DCMI 交叉验证口径）**；**风扇数据来源（v1.48.89：IPMI → lm-sensors → hwmon → SMBIOS Type 27 分层兜底，报告标注来源）**；**健康检查新增项（v1.48.90：GPU XID 错误历史 / CPU MCE（**v1.49.7 收紧判定口径**：只认真 MCE 标志 `Machine Check` / `Hardware Error` / `MCA: ` / `mce: [Hardware Error]`，并排除内核启动与驱动初始化噪音 `MCE: In-kernel MCE decoding enabled` / `mce: CPU supports` / `EDAC MC: Ver` / `EDAC MC*: Giving out device`——裸 `MCE:` 前缀曾把正常启动信息报成「检出 1 条机器检查异常」，实测导致整机误判「有条件通过」） / IB 链路误码（perfquery）****/ NVMe 错误日志 / RAID 缓存电池与写缓存策略——前五类为「曾出过事」的直接证据，验收中仅在检出时追加 WARN 项，不改变默认项数）**；SEL（v1.48.86 按「告警终态」判：`Asserted` 后无对应 `Deasserted` = 当前仍告警 → FAIL；成对自愈 → WARN 并标注自愈日期；旧实现把 Deasserted 行也算作 Critical）** / 固件合规 / OS-BMC 一致性 / PCIe 链路完整（v1.41.0）/ CPU 配置一致 / 内存容量一致 / 内存 ECC（v1.48.40 自洽校验）
