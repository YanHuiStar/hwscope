...
- **预检失败 ≠ 断网：先手工试一次真实 push（v1.48.92 立规）**：`git_push.sh` 的网络预检是 `curl --max-time N https://github.com` 快速判定（默认 15s，可用 `GIT_PUSH_PRECHECK_TIMEOUT` 覆盖）。**慢 ≠ 断**——实测同一时刻预检判「直连+代理均不可达」，紧接着手工 `git push` 约 20s 就推成功了。预检偏保守只是多等几秒，误判成「断网」却会**把有效推送挡在门口**，再把 PAUSE 纪律一叠加，这一轮推送就彻底卡住。**规则**：预检失败后**先手工跑一次** `timeout 60 git push origin main`——成功即完成推送（不必再走 git_push）；**只有真实 push 也失败**才上报用户检查代理节点/网络。历史上预检已因此误杀多次（超时 3s→5s→15s 三次放宽，判定方式 HEAD→GET 一次修正）。
