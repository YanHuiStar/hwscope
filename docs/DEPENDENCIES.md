...
>
> ✅ **国产五家（寒武纪/壁仞/摩尔线程/沐曦/天数智芯）识别已接入（v1.52.0）**：`detect_gpu_vendors` 厂商串映射已补齐，装上表工具即走对应厂商采集路径（全量日志 + 统一 CSV）。**厂商串【待真机核对】**——以真机 lspci vendor name 为准校准；未匹配时回落 generic lspci 兜底。
