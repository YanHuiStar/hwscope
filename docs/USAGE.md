...
17 个模块**同时并行启动**（默认）。模块日志**谁先跑完谁先输出**，因此屏幕上的段落顺序**不按模块序号**
（快的如 NVSM 秒级完成排前面，慢的 BMC/PSU 压轴）。这是刻意保留的实时反馈：慢机上能先看到快模块完成，
不至于以为卡死。

**正因为输出顺序随各模块耗时变化、同一台机器两次采集都可能不同**，v1.51.4 起在采集结束时追加一张
**按模块序号（01→16→99）的汇总表**，便于纵览与两次采集对照（顺序固定，可直接 diff）：

```
════════════ 采集汇总（按模块序号，与上方输出顺序无关）════════════
  模块           状态       耗时 文件数
  01/motherboard OK           1s      12
  02/cpu         OK           1s      12
  ...
  12/bmc         OK          60s      20
════════════════════════════════════════════════════════════════
...
顺序完全可控的写法是 `--serial`（串行，实时逐模块输出），代价是总耗时等于各模块累加。
另：TTY 下的转圈进度动画依赖 stdout 是终端，写日志/接管道时自动关闭——v1.51.4 修正了原先
因 `exec > >(tee ...)` 把 stdout 变成管道、导致该判断恒假、**动画从未显示过**的问题。
...
- 结果：`output/remote_output/<机器ID>/`；归档包 → `logs/remote_logs/`；v1.48.99 起回拉为"逐机器精准替换"（清空旧目录 + 归档校验 + 目录名护栏，杜绝旧文件残留被兜底误读）
