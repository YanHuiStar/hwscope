...
### git_push.sh 推送前检查（v1.51.5 起三条）

| 检查 | 作用 |
|------|------|
| **版本单调性** | 本地 `HWSCOPE_VERSION` < 远程 → 拒绝（防凭记忆回退版本）|
| **版本三处一致** | `hwscope.sh` 变量 / `hwscope.sh` 头部注释 / `README.md` 徽章 三者不一致 → **拒绝推送**，提示跑 `tools/sync_version.sh`（v1.51.3/.4 曾连续两次只改变量忘同步，徽章与注释停在旧版）|
| **SN 兜底** | 推送前跑 `sn_check.sh` 扫暂存内容与提交信息（`.git/hooks/` 不进仓库，换机器就没有）|

> 三条都放在 `git_push.sh` 而非 git hook：**推送是进公开仓库的唯一必经关口**，不依赖任何人（或任何 agent 会话）的自觉。
