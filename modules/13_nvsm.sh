...
# 说明：仅 NVIDIA MGX 认证整机（DGX OS / MGX 出厂镜像）预装，无 NVSM 则静默跳过。
#   v1.51.3 实测：标准 Ubuntu 24.04 上**不存在** nvsm（command -v 找不到、文件系统也没有、
#   apt 里无此包）。本项目目标机统一 Ubuntu，故本模块当前**恒走 SKIP 分支**；
#   保留是为了将来验 DGX 原厂整机（DGX OS）时可直接生效——那时才有真实数据。
# =============================================================================
