...
- v1.34.x — **Windows 原生远程采集**（tools/win/remote_collect.ps1/.bat：等价 remote_collect.sh，交互密码 + Windows OpenSSH + bsdtar，零新依赖）+ 多轮修复（cmd 引号地狱→分段 scp 流、ControlMaster 降级、拉取路径规范化、输出落 output/remote_output/<SN>/、鉴权重试 3 次、root 免 sudo）；`cleanup.sh/.ps1` 输出清理工具（尺寸预览 + 确认 + --force）
- v1.33.x — **验收 N/A 计数规则细化**（交付固有缺口不计数据不足：IB 线缆未插/无盘平台/无固件基线；有盘无 SMART 仍计入）、OS-BMC 校验默认关闭 + `--bmc-verify` 独立核验报告、IB 线缆配对条件驱动（有链接无数据=采集缺口计入，未插线=平台形态不计）、GPU 数值 guard 修前导空格、perftest 命令修正
- v1.31.x — `remote_collect.sh` SSH 远程采集（tar 打包 + 交互密码默认 + ControlMaster 复用）、`dhcp_server.sh` dnsmasq 封装（含租约导出/对账）、`power_monitor.sh` 连续采样 + kWh、`report_server.sh` Web 预览、`remote_batch.sh` 多机运维、`fw_baseline_import` 基线自动导入
