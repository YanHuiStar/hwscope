...
- **多厂商适配器（v1.47.0；昇腾/Intel/国产待真机验证；**国产五家识别 v1.52.0 接入，厂商串待真机核对，见 ARCHITECTURE §模块架构**）**：昇腾（npu-smi + HCCS 拓扑/health 日志）/ Intel（xpu-smi）/ 国产（cnmon·bmt-smi·mthreads-gmi·mx-smi·ix-smi）全量日志落盘 + 统一 `gpu_inventory.csv`（lspci 层，PCIe 链路判定可用）；厂商 SMI 解析标注【待真机校准】；无工具自动 lspci 兜底，卡不丢失
