...
>
> ⚠️ **国产五家（寒武纪/壁仞/摩尔线程/沐曦/天数智芯）接入现状（v1.51.11 审查确认）**：适配器与工具探测已实现，但平台识别层（`detect_gpu_vendors` 的 lspci 厂商串 case）尚未匹配这五家——**当前即使装了 cnmon/bmt-smi 等工具，采集仍走 generic lspci 兜底**（不会进入对应厂商采集路径）。上表工具可照常安装备用，识别映射补齐后即自动生效。
