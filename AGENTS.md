...
- `modules/gpu/` — 显卡多厂商适配器框架（`lib.sh` 统一接口 + 10 个 `adapter_<vendor>.sh`：nvidia / amd / intel / ascend / cambricon / metax / moorethreads / iluvatar / biren / generic）；由 `04_gpu.sh` 按 `detect_gpu_vendors` 结果 source，**非独立采集模块**（不在 MODULES 注册表，`--modules gpu` 走 04_gpu.sh）
...
- `test/` — 硬件压测（**⚠️ 状态：占位——未完成、未经检验**：测试项目未定稿、未在真机验证，**勿据此下验收结论**）（聚合与实现解耦，v1.41.3）：`test_all.sh` 纯聚合入口（菜单/--all）+ `test/<组件>/` 单工具脚本（可独立执行，如 `test/cpu/cpu_stress_ng.sh`）+ `test/lib/test_common.sh` 公共库（落盘 `logs/test/<SN>/`）；只测不改
...
- **IPMI 超时统一为可配置的三级 `IPMI_TIMEOUT*`（v1.48.91 立规，v1.49.8/.9 分为三级；**代码现值：中 90s、FAST 30s、SLOW 240s**，本节早期写的"默认 30s"已过时）**：此前各模块硬编码 `timeout 10`（10_psu/11_fan/12_bmc/16_power），对 BMC 响应慢的平台不够——实测技嘉 B200 五台机上各有 **10~13 个 IPMI 命令在 10s 内全部超时**（`ipmi_sensors`/`sdr`/`fan_*`/`psu_*`/`sensors_temp` 全线 `exit=124`），导致风扇/温度/PSU 冗余三项落成「采集失败」，其中一台因此 N/A 达 4 项、跨过「≥4 项 = 数据不足」阈值而无法验收。**规则**：统一由 `lib/common.sh` 提供：`IPMI_TIMEOUT`（默认 90，sel/fru 类）、`IPMI_TIMEOUT_FAST`（默认 30，chassis/lan/mc info 类）、`IPMI_TIMEOUT_SLOW`（默认 240，`sensor list`/`sdr list` 逐条读 SDR 的慢命令）；模块内写 `timeout ${IPMI_TIMEOUT:-90}` 之类保留兜底（独立跑模块时不 source common.sh 也安全）；新增 IPMI 命令一律引用该变量，禁止再写死数值。**配套（v1.49.20）**：单模块超时 `MODULE_TIMEOUT` 默认由 300 提到 600——`sensor list`+`sdr list` 两份共享快照在慢 BMC 上本身就要 240+240s，300s 必然把模块杀在中途。**调大默认值的代价可控**：`timeout` 只对**卡住的命令**生效，正常命令 1s 内返回就不会等满 30s，所以对健康 BMC 的采集耗时几乎无影响，只让慢 BMC 有机会把数据取回来。
