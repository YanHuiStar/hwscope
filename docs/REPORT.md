...
- **18 项判定**：GPU PCIe / NVLink（v1.48.40 能力感知：消费级无 NVLink 判 N/A）/ DCGM（消费级 N/A）/ VBIOS / 内存速率 / IB 线缆 / 磁盘寿命 / SMART / 电源冗余 / 温度 / SEL / 风扇冗余（v1.36.0）/ 固件合规 / OS-BMC 一致性 / PCIe 链路完整（v1.41.0）/ CPU 配置一致 / 内存容量一致 / 内存 ECC（v1.48.40 自洽校验）
