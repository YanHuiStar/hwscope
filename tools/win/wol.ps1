...
.PARAMETER Mac
  目标 MAC 地址（必填，支持 aa-bb-cc-dd-ee-ff 或 AABBCCDDEEFF 格式）
.PARAMETER Broadcast
  广播地址（默认 255.255.255.255；跨网段唤醒可指定目标网段广播，如 192.168.1.255）
.PARAMETER Port
  唤醒端口（默认 9；部分设备用 7）
.EXAMPLE
  .\wol.ps1 -Mac aa-bb-cc-dd-ee-ff
  .\wol.ps1 -Mac AABBCCDDEEFF -Broadcast 192.168.1.255
#>
param(
...
if ($hex.Length -ne 12) {
    Write-Host "MAC 格式错误: $Mac（应为 12 位十六进制，如 aa-bb-cc-dd-ee-ff）" -ForegroundColor Yellow
    exit 1
}
