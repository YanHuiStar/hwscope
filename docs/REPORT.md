...
- **硬件概览（配置单）**：自动生成自检测数据（准系统/CPU/内存/GPU模组/计算网卡/DPU/网卡&端口/存储/电源模块/系统管理），可对照采购配置单核对。**计算网卡按「GPU 直连」归类（v1.48.85）**——判据是 `nvidia-smi topo -m` 的 `PIX`（与 GPU 同 PCIe switch），**不是当前运行协议**（IB/ETH 模式随时可配，物理直连固定）；**DPU（BlueField 系列）单列一行**，判定顺序 DPU → GPU直连 → 其余；**数量单位是「口」**（每行一个网络端口，双口卡占两行）。**GPU 直连判据 = `PIX` 或 `PXB` + 卡型过滤（ConnectX-6/7/8、BlueField）（v1.48.86）**——两者都属不经 CPU 的本地连接；**无 topo 数据的平台（AMD/昇腾无 nvidia-smi topo）回退按 IB 协议口判定**
- **验收判定项（项数随平台能力裁剪，实测 B300 为 14 项）**：GPU PCIe / NVLink（v1.48.40 能力感知：消费级无 NVLink 判 N/A）/ DCGM（消费级 N/A）/ VBIOS / 内存速率 / IB 线缆 / 磁盘寿命 / SMART / 电源冗余 / 温度 / ****风扇（v1.48.88：`FAN_DATA_OK` 区分「采集失败」与「平台无风扇」——命令超时致日志为空时显示 `N/A（未取到数据）` 并计入数据不足，不再误报「无风扇」）**；**CPU 功耗（v1.48.89：采集端读 RAPL 两次采样算 ΔE/Δt，报告端单列，可与 DCMI 交叉验证口径）**；**风扇数据来源（v1.48.89：IPMI → lm-sensors → hwmon → SMBIOS Type 27 分层兜底，报告标注来源）**；**健康检查新增项（v1.48.90：GPU XID 错误历史 / CPU MCE / IB 链路误码（perfquery）****/ NVMe 错误日志 / RAID 缓存电池与写缓存策略——前五类为「曾出过事」的直接证据，验收中仅在检出时追加 WARN 项，不改变默认项数）**；SEL（v1.48.86 按「告警终态」判：`Asserted` 后无对应 `Deasserted` = 当前仍告警 → FAIL；成对自愈 → WARN 并标注自愈日期；旧实现把 Deasserted 行也算作 Critical）** / 固件合规 / OS-BMC 一致性 / PCIe 链路完整（v1.41.0）/ CPU 配置一致 / 内存容量一致 / 内存 ECC（v1.48.40 自洽校验）
