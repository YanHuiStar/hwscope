...
- **18 项判定**：GPU PCIe / NVLink（v1.48.40 能力感知：消费级无 NVLink 判 N/A）/ DCGM（消费级 N/A）/ VBIOS / 内存速率 / IB 线缆 / 磁盘寿命 / SMART / 电源冗余 / 温度 / ****风扇（v1.48.88：`FAN_DATA_OK` 区分「采集失败」与「平台无风扇」——命令超时致日志为空时显示 `N/A（未取到数据）` 并计入数据不足，不再误报「无风扇」）**；SEL（v1.48.86 按「告警终态」判：`Asserted` 后无对应 `Deasserted` = 当前仍告警 → FAIL；成对自愈 → WARN 并标注自愈日期；旧实现把 Deasserted 行也算作 Critical）** / 风扇冗余（v1.36.0）/ 固件合规 / OS-BMC 一致性 / PCIe 链路完整（v1.41.0）/ CPU 配置一致 / 内存容量一致 / 内存 ECC（v1.48.40 自洽校验）
