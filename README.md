...
- **GPU 魔改识别**：内置 60+ NVIDIA + AMD Instinct + 昇腾/Intel/国产型号额定显存规格库（昇腾/Intel/国产待真机验证），检测值交叉验证，魔改/伪装卡自动 `⚠️` 提示
